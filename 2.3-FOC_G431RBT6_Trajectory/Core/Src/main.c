...
int main(void)
{
  ...
  float encoder_zero = 5.34067; // 电机0 角° (这只是随意测得的大概值)
  float Kt = 0.03;              // 电机力矩常数
  double theta = 0;             // 编码器角度映射后值
  double last_theat = 0;
  double Iq_max = 0.032 / Kt / MAX_CURRENT; // 最大力矩电流 fabsf()
  double angle_step = 2 * PI / 512;         // 角度分值
  double half_angle_step = angle_step / 2;
  double next_theta; // 下一步要到达值
  double sect;
  double temp;
  double diff;
  int first = 1;
  ...
  {
    /* USER CODE END WHILE */
    theta = encoder_angle - encoder_zero;
    if (theta < 0)
      theta += 2 * PI;
    // if (first)
    // {
    ...
    //   last_theat = theta;
    // }
    // diff = theta - last_theat;
    // if (diff > PI)
    //   diff -= 2 * PI;
    // else if (diff < (-1.0 * PI))
    //   diff += 2 * PI;

    // temp = theta / angle_step;
    // sect = floor(temp);

    // temp = (temp - sect) * angle_step;
    // if (diff > 0 && temp > half_angle_step)
    //   next_theta = (sect + 1) * angle_step;
    // else if (diff < 0 && temp > half_angle_step)
    //   next_theta = (sect - 1) * angle_step;
    // else
    //   next_theta = sect * angle_step;

    motor_control_context.torque_norm_q = sinf(theta - 0.5 * PI) * Iq_max - 0.5 * motor_speed; // 不同的力矩有方向 错位90°
